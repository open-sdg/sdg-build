...
class MetadataReportService(Loggable):
    ...
    def get_indicators_dataframe(self):
        ...
        df = pd.DataFrame(rows, columns=['Indicator', 'Data non-statistical', 'Computation units', 'Data show map', 'National geographical coverage', 'Reporting status', 'UN designated tier', 'UN custodian agency', 'Graph type'])
        if not df.empty:
