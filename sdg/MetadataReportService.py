...
class MetadataReportService(Loggable):
    ...
    def get_indicators_dataframe(self):
        ...
        df = pd.DataFrame(rows, columns=['Indicator', 'Reporting status', 'UN designated tier', 'Data non-statistical', 'Computation units', 'Data show map', 'National geographical coverage', 'UN custodian agency', 'Graph type'])
        if not df.empty:
