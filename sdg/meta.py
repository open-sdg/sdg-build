...
def read__yamlmd_meta(inid, git=True, src_dir='', git_data_dir=None):
...
def read__csv_meta(inid, git=True, src_dir='', git_data_dir=None):
    """Perform pre-processing for the metadata files"""
    status = True
    # Read and write paths may be different
    fr = input_path(inid, ftype='meta', src_dir=src_dir)

    metadata_mapping = os.path.join('metadata-mapping.csv')
    meta_csv = pd.read_csv(fr)
