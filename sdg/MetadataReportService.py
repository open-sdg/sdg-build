...
class MetadataReportService(Loggable):
    ...
    def get_indicators_dataframe(self):
        ...
        df = pd.DataFrame(rows, columns=['Indicator', 'Computation units', 'Data non-statistical', 'Data show map', 'National geographical coverage', 'Reporting status', 'UN designated tier', 'Graph type', 'UN custodian agency'])
        if not df.empty:
